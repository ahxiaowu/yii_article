...
class Article extends CActiveRecord {
	...
	public function common(){
		$commArr = array();
		$sql = "select cid,cname from {{cate}} limit 5";
		$commArr['nav'] = Cate::model()->findAllBySql($sql);
		
		$sql = "select aid,title from {{article}} order by pubtime desc limit 5";
		$commArr['title'] = self::model()->findAllBySql($sql);
		return $commArr;
	}
